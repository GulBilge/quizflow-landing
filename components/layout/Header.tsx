...
export default function Header({ user }: HeaderProps) {
    ...
    return (
        <>
            <header className="sticky top-0 z-40 w-full bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 transition-colors duration-300">
                <div className="flex h-16 items-center justify-between px-4 md:px-8">
                    ...
                    <div className="flex flex-1 items-center justify-end gap-2 sm:gap-4">
                        {/* Search Bar */}
                        <div className={cn(
                            "relative flex items-center transition-all duration-300 max-w-full",
                            isSearchFocused ? "flex-1 max-w-[calc(100vw-120px)] sm:max-w-sm lg:max-w-md" : "w-10 lg:w-48 xl:w-64"
                        )}>
            ...
            {/* Mobile Sidebar (Moved outside header for stacking context) */}
            <MobileSidebar
